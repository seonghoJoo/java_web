    ...
    <ul id="dailySchedulesList">
      ...
      <li class="daily_schedule_item">
        <dl>
          ...
          <dd><@=daily.start@> 시작</dd>
          <@ if(daily.extendedProps.participantsNum!==undefined){ @>
          <dt class="schedule_rsvp screen_out">참여여부</dt>
          <dd>참석 <@=daily.extendedProps.participantsNum@></dd>
          <dd>불참석 <@=daily.extendedProps.nonParticipantsNum@></dd>
          <@} else{} @>
        </dl>
      </li>
      <@})@>
...
<script>
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    dateClick: function(info) {
      console.log(info);
      alert(info.date.getDate());
      getDailySchedulesList(info.date);
  ...
  $newWholeCalendar.click(function (e) {//일정 전체보기 탭 클릭시
    ...
    function getDailySchedulesList(date) {

      $.ajax({
        url:"ajax/dailySchedules.json",
        data:{date:date},
        error:function() {
        ...
        success:function (json) {
          ...
          $(".daily_schedule_box").html(dailySchedulesListTmpl({date :date.getDate(), day:dayArray[date.getDay()], dailySchedules:json}));
